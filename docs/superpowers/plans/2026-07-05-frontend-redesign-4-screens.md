...
test("portfolioValueSeries último punto = Σ shares·currentPrice", () => {
  ...
  const expected = assets.reduce(
    (acc, a) => acc + a.shares * a.currentPrice,
    0,
  );
...
const EXPENSE_CATS: { id: string; base: number; desc: string[] }[] = [
  {
    id: "alimentacion",
    base: 55,
    desc: ["Supermercado", "Frutería", "Panadería"],
  },
...
export function priceHistory(assetId: string, days = HISTORY_DAYS): Point[] {
  ...
  return logs.map((s, i) => ({
    t: t0 + i * DAY,
    v: asset.currentPrice * Math.exp(s - last),
  }));
}
...
export function distribucionPorClase(): Segment[] {
  ...
  return [...totals.entries()]
    .map(([label, value]) => ({
      label,
      value,
      color: CLASS_COLOR[label] ?? "var(--cat-6)",
    }))
    .sort((a, b) => b.value - a.value);
}
...
export function patrimonioSeries(range: Range): Point[] {
  ...
  const out: Point[] = cartera.map((p) => {
    while (
      idx < sorted.length &&
      +new Date(sorted[idx].date) <= p.t + 86_400_000 - 1
    ) {
...
const Placeholder = ({ name }: { name: string }) => (
  <div className="card">{name} — en construcción</div>
);
                ...
                <li key={m.id} className="movement">
                  ...
                  <span className="mv-main">
                    ...
                    <span className="mv-meta">
                      {catName.get(m.categoryId)} ·{" "}
                      {formatRelativeDateTime(m.date)}
                    </span>
                  </span>
                  <span
                    className={`mv-amount num ${income ? "tone-positive" : "tone-negative"}`}
                  >
    ...
    <>
      ...
      >
        <div className="cartera-hero-head">
          ...
          <span
            className={`hero-delta num ${total.pnl >= 0 ? "tone-positive" : "tone-negative"}`}
          >
            {total.pnl >= 0 ? "+" : ""}
            ...
            {assets.map((a) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="ta-r num">
                    {formatCurrency(m.marketValue)}
                  </span>
                  <span className={`ta-r num ${tone}`}>
...
export function AssetDetail() {
  ...
  return (
    <>
      ...
      >
        <div className="cartera-hero-head">
          <p className="hero-figure num">
            {formatCurrency(asset.currentPrice)}
          </p>
          <span
            className={`hero-delta num ${variacion >= 0 ? "tone-positive" : "tone-negative"}`}
          >
            {variacion >= 0 ? "+" : ""}
            {formatCurrency(variacion)} ({formatPercent(variacionPct)}) ·{" "}
            {range}
          </span>
        </div>
        <AreaTrend data={serie} height={300} />
          ...
          {txs.map((t) => {
            ...
            return (
              <div key={t.id} className="tx-row">
                ...
                <span
                  className={
                    t.type === "compra" ? "tone-positive" : "tone-negative"
                  }
                >
                  {t.type}
                </span>
