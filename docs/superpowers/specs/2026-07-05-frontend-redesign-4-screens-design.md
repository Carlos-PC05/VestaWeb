...
3. **Navegación:** 3 items laterales (Dashboard, Movimientos, Cartera). El detalle de activo es una ruta anidada en Cartera. El toggle de tema claro/oscuro vive en la **topbar**.
...
- **Sidebar** izquierda: logo "Vesta" + 3 items de nav con icono (iconos inline en `lib/icons.tsx`). Item activo resaltado con acento.
- **Topbar**: título de la pantalla actual (izquierda) + toggle de tema claro/oscuro (derecha).
...
- Pantalla de Ajustes dedicada (el único ajuste, el tema, vive en la topbar).
